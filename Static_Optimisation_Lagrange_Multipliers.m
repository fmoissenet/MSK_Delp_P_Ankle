...
% Initialisation
W = eye(43+21); % Number of muscles + number of Lagrange multipliers

% % Condition 1 (Moissenet et al. 2014)
% W(43+1:43+21,43+1:43+21) = zeros(21,21);

% Condition 2 (Moissenet et al. 2014)
% Ankle contact about X axis
W(43+2,43+2) = 1e-0; % Ankle contact about Y axis 
% Ankle contact about Z axis
W(43+4,43+4) = 1e-6; % TiCaL
W(43+5,43+5) = 1e-6; % CaFiL 
W(43+6,43+6) = 2e-0; % Knee medial contact 
W(43+7,43+7) = 4e-0; % Knee lateral contact 
W(43+8,43+8) = 1e-6; % ACL 
W(43+9,43+9) = 1e-6; % PCL 
% MCL
% Patellar contact about X axis
% Patellar contact about Y axis
% Patellar contact about Z axis
% PT 
% Hip contact about X axis 
W(43+16,43+16) = 1e-0; % Hip contact about Y axis 
% Hip contact about Z axis 
% Foot axial 
W(43+19,43+19) = 1e-6; % Tibia axial 
% Patella axial 
W(43+21,43+21) = 1e-6; % Femur axial 
...
for i = 1:n
    ...
    % ACL
    % PCL
    Xmin(43+10,1) = -Inf; % MCL
    Xmin(43+11,1) = -Inf; % Patellar contact about X axis
    Xmin(43+12,1) = -Inf; % Patellar contact about Y axis
    Xmin(43+13,1) = -Inf; % Patellar contact about Z axis
    % PT
    Xmin(43+15,1) = -Inf; % Hip contact about X axis
    % Hip contact about Y axis
    Xmin(43+17,1) = -Inf; % Hip contact about Z axis
    Xmin(43+18,1) = -Inf; % Foot axial
    % Tibia axial
    Xmin(43+20,1) = -Inf; % Patella axial
    % Femur axial
    
    % Upper bounds
    Xmax = Inf(43 + 21,1);
%     Xmax = [Model.Fmax(1:43,1);Inf(21,1)];
%     Xmax = [Model.PCSA(1:43,1)*61;Inf(21,1)]; % Muscle stress of 61 N/cm^2
    
    % ---------------------------------------------------------------------
    % Run optimization (fmincon)
    % ---------------------------------------------------------------------
    options = optimset('Display','off','MaxIter',100,'LargeScale','off','TolFun',0.1,'TolX',1e-10,'TolCon',1e-10,...
        'algorithm','sqp','GradObj','on'); % active-set
    C = @(X)Criterion_Lagrange_Multipliers(X,W); % Anonymous function (to pass extra parameters W)
    [X,~,exitflag] = fmincon(C,Xini,[],[],Aeq,Beq,Xmin,Xmax,[],options);
    Model.X(:,1,i) = X;
    Model.flag(i) = exitflag;
