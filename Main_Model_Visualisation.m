...
% Ankle
% Lateral_centre_calcaneus = Lateral_contact_tibia
...
% Insertion of Calcaneum / Tibia Ligament
% V22: virtual marker 2 of segment 2
V22 = Mprod_array3(repmat(([Segment(2).nV(1,2)*eye(3),...
    (1 + Segment(2).nV(2,2))*eye(3), ...
    - Segment(2).nV(2,2)*eye(3), ...
    Segment(2).nV(3,2)*eye(3)]),[1 1 n]),Segment(2).Q);  
V22x = permute(V22(1,1,ni),[3,2,1]);
V22y = permute(V22(2,1,ni),[3,2,1]);
V22z = permute(V22(3,1,ni),[3,2,1]);
plot3(V22x,V22y,V22z,'^b');
% Origin of Calcaneum / Tibia Ligament
% V23: virtual marker 2 of segment 3
V23 = Mprod_array3(repmat(([Segment(3).nV(1,2)*eye(3),...
    (1 + Segment(3).nV(2,2))*eye(3), ...
    - Segment(3).nV(2,2)*eye(3), ...
    Segment(3).nV(3,2)*eye(3)]),[1 1 n]),Segment(3).Q);
V23x = permute(V23(1,1,ni),[3,2,1]);
V23y = permute(V23(2,1,ni),[3,2,1]);
V23z = permute(V23(3,1,ni),[3,2,1]);
plot3(V23x,V23y,V23z,'^r');
plot3([V22x,V23x]',[V22y,V23y]',[V22z,V23z]','--k');
% Insertion of Calcaneum / Fibula Ligament
% V32: virtual marker 3 of segment 2
V32 = Mprod_array3(repmat(([Segment(2).nV(1,3)*eye(3),...
    (1 + Segment(2).nV(2,3))*eye(3), ...
    - Segment(2).nV(2,3)*eye(3), ...
    Segment(2).nV(3,3)*eye(3)]),[1 1 n]),Segment(2).Q);  
V32x = permute(V32(1,1,ni),[3,2,1]);
V32y = permute(V32(2,1,ni),[3,2,1]);
V32z = permute(V32(3,1,ni),[3,2,1]);
plot3(V32x,V32y,V32z,'^b');
% Origin of Calcaneum / Fibula Ligament
% V33: virtual marker 3 of segment 3
V33 = Mprod_array3(repmat(([Segment(3).nV(1,3)*eye(3),...
    (1 + Segment(3).nV(2,3))*eye(3), ...
    - Segment(3).nV(2,3)*eye(3), ...
    Segment(3).nV(3,3)*eye(3)]),[1 1 n]),Segment(3).Q);  
V33x = permute(V33(1,1,ni),[3,2,1]);
V33y = permute(V33(2,1,ni),[3,2,1]);
V33z = permute(V33(3,1,ni),[3,2,1]);
plot3(V33x,V33y,V33z,'^r');
plot3([V32x,V33x]',[V32y,V33y]',[V32z,V33z]','--k');
...
quiver3(V43x,V43y,V43z,n13x,n13y,n13z,0.15,'--k');
...
quiver3(V53x,V53y,V53z,n23x,n23y,n23z,0.15,'--k');
